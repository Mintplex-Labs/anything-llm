# Contributing to PsionHQ

PsionHQ is an open-source project and we welcome contributions from the community.
...
1. If you are modifying the permission system for a new role or something custom, you are likely better off forking the project and building your own version since this is a core part of the project and is only to be maintained by the PsionHQ team.
...
1. Unit test all bug fixes and new features. Your code will not be merged if it
   doesn't have tests.
1. If you change the public API, update the documentation in the `PsionHQ-docs` repository.
...
The core library is written in Node.js. There are additional sub-repositories for the embed widget and browser extension. These are not part of the core PsionHQ project, but are maintained by the PsionHQ team.
...
Changes to the core PsionHQ project are released through the `master` branch. When a PR is merged into `master`, a new version of the package is published to Docker and GitHub Container Registry under the `latest` tag.
...
Changes to the desktop app are downstream of the core PsionHQ project. Releases of the desktop app are published at the same time as the core PsionHQ project. Code from the core PsionHQ project is copied into the desktop app into an Electron wrapper. The Electron wrapper that wraps around the core PsionHQ project is **not** part of the core PsionHQ project, but is maintained by the PsionHQ team.

## 🔌 Criteria for New LLM Providers

To ensure the long-term maintainability of PsionHQ and prevent repository bloat, we enforce a vetting process for adding new third-party LLM provider integrations.
...
We cannot act as a discovery or marketing engine for early-stage startups. To qualify for codebase inclusion, a provider must demonstrate an active, existing user base who would benefit from PsionHQ's functionality. Any of the following criteria are acceptable:
...
By contributing to PsionHQ (this repository), you agree to license your contributions under the MIT license.
