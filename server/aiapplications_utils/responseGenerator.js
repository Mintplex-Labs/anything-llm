...
async function generateAllRelatedQuestions(query, ai_response) {
    const ai = new GoogleGenAI({
        vertexai: true,
        project: project_id,
        location: "us-central1"
    ...
    const response = await ai.models.generateContent({
        ...
        config: {
          ...
          systemInstruction: [
            {
              text: `You are a helpful assistant that generates related questions to a given query. Given the prompt and the response that was generated by the LLM for the prompt, generate 3 related questions that a user might want to ask next.`,
            }
        ],
        },
      });
