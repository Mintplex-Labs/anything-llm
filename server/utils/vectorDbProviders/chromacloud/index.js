...
const ChromaCloud = {
  ...
  similarityResponse: async function ({
    ...
    response.ids[0].forEach((_, i) => {
      ...
      if (
        ...
        console.log(
          "ChromaCloud: A source was filtered from context as it's parent document is pinned."
        );
        return;
      }
  ...
  namespaceExists: async function (client, namespace = null) {
    ...
    const collection = await client
      .getCollection({ name: this.normalize(namespace) })
      .catch((e) => {
        console.error("ChromaCloud::namespaceExists", e.message);
        return null;
      });
    return !!collection;
  },
  ...
  addDocumentToNamespace: async function (
    ...
    try {
      ...
      if (!skipCache) {
        ...
        if (cacheResult.exists) {
          ...
          for (const chunk of chunks) {
            ...
            // Before sending to ChromaCloud and saving the records to our db
            // we need to assign the id of each chunk that is stored in the cached file.
            chunk.forEach((chunk) => {
              const id = uuidv4();
              const { id: _, ...metadata } = chunk.metadata;
            ...
            if (!additionResult)
              throw new Error("Error embedding into ChromaCloud", additionResult);
          }
      ...
      if (vectors.length > 0) {
        const chunks = [];
        console.log("Inserting vectorized chunks into ChromaCloud collection.");
        ...
        try {
          ...
          console.error("Error adding to ChromaCloud:", error);
          throw new Error(`Error embedding into ChromaCloud: ${error.message}`);
        }
