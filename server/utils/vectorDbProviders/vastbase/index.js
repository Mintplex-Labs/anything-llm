...
const Vastbase = {
  ...
  /**
   * Get the table name for the Vastbase database.
   * - Defaults to "anythingllm_vectors" if no table name is provided.
   * @returns {string}
   */
  tableName: () => process.env.VASTBASE_TABLE_NAME || "anythingllm_vectors",

  /**
   * Get the connection string for the Vastbase database.
  ...
  createTableSql: (dimensions) =>
    `CREATE TABLE IF NOT EXISTS "${Vastbase.tableName()}" (id UUID PRIMARY KEY, namespace TEXT, embedding floatvector(${Number(dimensions)}), metadata JSONB, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)`,

  log: function (message = null, ...args) {
    
    console.log(`\x1b[35m[Vastbase]\x1b[0m ${message}`, ...args);
  },
  ...
  client: function (connectionString = null) {
    return new vsql.Client({
      connectionString: connectionString || Vastbase.connectionString(),
    });
  },

  /**
   * Validate the existing embedding table schema.
   * @param {vsql.Client} pgClient
  ...
  validateExistingEmbeddingTableSchema: async function (pgClient, tableName) {
    ...
    this.log(
      `✅ The vastbase table '${tableName}' was found and meets the minimum expected schema for an embedding table.`
    );
    return true;
  },
  ...
  validateConnection: async function ({
    ...
    try {
      const timeoutPromise = new Promise((resolve) => {
        setTimeout(() => {
          resolve({
            error: `Connection timeout (${(Vastbase.connectionTimeout / 1000).toFixed(0)}s). Please check your connection string and try again.`,
            success: false,
          });
        }, Vastbase.connectionTimeout);
      });
   ...
   * @returns {Promise<vsql.Client>}
   */
  connect: async function () {
    if (!Vastbase.connectionString())
      throw new Error("No connection string provided");
    if (!Vastbase.tableName()) throw new Error("No table name provided");
  ...
  dbTableExists: async function () {
    ...
    try {
      ...
      const tableExists = tables.rows.some(
        (row) => row.tablename === Vastbase.tableName()
      );
      return !!tableExists;
    } catch (err) {
      return false;
  ...
  totalVectors: async function () {
    ...
    try {
      ...
      const result = await connection.query(
        `SELECT COUNT(id) FROM "${Vastbase.tableName()}"`
      );
  ...
  // Distance for cosine is just the distance for floatvector.
  ...
  namespaceCount: async function (namespace = null) {
    ...
    try {
      connection = await this.connect();
      const result = await connection.query(
        `SELECT COUNT(id) FROM "${Vastbase.tableName()}" WHERE namespace = $1`,
        [namespace]
      );
      return result.rows[0].count;
    } catch (err) {
  ...
  /**
   * Performs a SimilaritySearch on a given Vastbase namespace.
   * @param {Object} params
   * @param {vsql.Client} params.client
  ...
  similarityResponse: async function ({
    ...
    const response = await client.query(
      `SELECT embedding ${this.operator.cosine} $1 AS _distance, metadata FROM "${Vastbase.tableName()}" WHERE namespace = $2 ORDER BY _distance ASC LIMIT $3`,
   ...
   * @param {Object} params
   * @param {vsql.Connection} params.connection
  ...
  updateOrCreateCollection: async function ({
    ...
    try {
      ...
      for (const submission of submissions) {
        const embedding = `[${submission.vector.map(Number).join(",")}]`; // stringify the floatvecotr
        const sanitizedMetadata = this.sanitizeForJsonb(submission.metadata);
        await connection.query(
          `INSERT INTO "${Vastbase.tableName()}" (id, namespace, embedding, metadata) VALUES ($1, $2, $3, $4)`,
  ...
  namespaceExists: async function (connection, namespace = null) {
    if (!namespace) throw new Error("No namespace provided");
    const result = await connection.query(
      `SELECT COUNT(id) FROM "${Vastbase.tableName()}" WHERE namespace = $1 LIMIT 1`,
      [namespace]
    );
  ...
  deleteVectorsInNamespace: async function (connection, namespace = null) {
    if (!namespace) throw new Error("No namespace provided");
    await connection.query(
      `DELETE FROM "${Vastbase.tableName()}" WHERE namespace = $1`,
      [namespace]
    );
    return true;
  },
  ...
  addDocumentToNamespace: async function (
    ...
    try {
      ...
      if (vectors.length > 0) {
        ...
        this.log("Inserting vectorized chunks into Vastbase collection.");
        await this.updateOrCreateCollection({
          connection,
          submissions,
          namespace,
  ...
  deleteDocumentFromNamespace: async function (namespace, docId) {
    ...
    try {
      ...
      if (!exists)
        throw new Error(
          `Vastbase:deleteDocumentFromNamespace - namespace ${namespace} does not exist.`
        );
      ...
      try {
        ...
        for (const vectorId of vectorIds)
          await connection.query(
            `DELETE FROM "${Vastbase.tableName()}" WHERE id = $1`,
            [vectorId]
          );
        await connection.query(`COMMIT`);
      } catch (err) {
  ...
  reset: async function () {
    ...
    try {
      connection = await this.connect();
      await connection.query(`DROP TABLE IF EXISTS "${Vastbase.tableName()}"`);
      return { reset: true };
    } catch (err) {
      return { reset: false };
    } finally {
...
module.exports.Vastbase = Vastbase;
