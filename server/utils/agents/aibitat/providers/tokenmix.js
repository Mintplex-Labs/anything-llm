...
const { TokenMixLLM } = require("../../../AiProviders/tokenmix/index.js");

/**
 * The agent provider for the TokenMix provider.
 ...
 */
class TokenMixProvider extends InheritMultiple([Provider, UnTooled]) {
  model;

  constructor(config = {}) {
    const { model = "deepseek/deepseek-v4-flash" } = config;
    super();
    this.providerTag = "tokenmix";
   ...
   * Checks the TokenMix model capabilities and caches the result.
   * @returns {Promise<boolean>}
   */
  async supportsNativeToolCalling() {
    if (this.optsOutOfNativeToolCallingViaEnv(this.providerTag)) return false;
    if (this._supportsToolCalling !== null) return this._supportsToolCalling;
    const tokenmix = new TokenMixLLM(null, this.model);
    const capabilities = await tokenmix.getModelCapabilities();
    ...
    return await this.client.chat.completions
      ...
      .then((result) => {
        if (!result.hasOwnProperty("choices"))
          throw new Error("TokenMix chat: No results!");
        if (result.choices.length === 0)
          throw new Error("TokenMix chat: No results length!");
   ...
   * Stubbed since TokenMix has no cost basis.
...
module.exports = TokenMixProvider;
