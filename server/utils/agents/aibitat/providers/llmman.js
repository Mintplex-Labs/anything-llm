 ...
 */
class LlmmanProvider extends InheritMultiple([Provider, UnTooled]) {
  model;
  ...
  async supportsNativeToolCalling() {
    ...
    const llmman = new LlmmanLLM(null, this.model);
    const capabilities = await llmman.getModelCapabilities();
  ...
  get queryOptions() {
    this.providerLog(
      `${this.model} is using a max context window of ${LlmmanLLM.promptWindowLimit(this.model)}/${LlmmanLLM.maxContextWindow(this.model)} tokens.`
    );
    return {
  ...
  /**
   * Parse a data URL into base64 data for the images field
  ...
  /**
   * Override formatMessageWithAttachments for the Ollama API's format.
   * The Ollama API expects images in a separate 'images' array with base64 data (no data URI prefix),
   * not the OpenAI-style content array format.
   * **This is only used for the untooled fallback mode.**
   * @param {Object} message - Message with potential attachments
   * @returns {Object} Formatted message for the Ollama API
   */
   ...
   * originalFunctionCall metadata) into the Ollama API tool-calling message format
   * (assistant tool_calls + role:"tool" result pairs).
   * Handles image attachments for vision/multimodal support.
   * @param {Array} messages
   * @returns {Array}
   */
  #formatMessagesForApiTools(messages) {
    const formatted = [];
   ...
   * Ollama API client + UnTooled prompt-based approach.
   *
  ...
  async stream(messages, functions = [], eventHandler = null) {
    ...
    if (useNative) {
      this.providerLog(
        "LlmmanProvider.stream (tooled) - will process this chat completion."
      );
      this.resetUsage();
      await LlmmanLLM.cacheContextWindows();
      const msgUUID = v4();
      const formattedMessages = this.#formatMessagesForApiTools(messages);
      ...
      for await (const chunk of stream) {
        // Capture usage from final chunk (usage arrives when done=true)
        if (chunk.done === true) {
          this.recordUsage({
    ...
    // Fallback: UnTooled prompt-based approach via the Ollama API client
    this.providerLog(
      "LlmmanProvider.stream - will process this chat completion."
    );
  ...
  async complete(messages, functions = []) {
    ...
    if (useNative) {
      ...
      // Record usage (the Ollama API reports prompt_eval_count/eval_count)
      this.recordUsage({
    ...
    // Fallback: UnTooled prompt-based approach via the Ollama API client
    this.providerLog(
      "LlmmanProvider.complete - will process this chat completion."
    );
    // eslint-disable-next-line
    try {
      let completion = { content: "" };
   ...
   * Stubbed since a local runner has no cost basis.
