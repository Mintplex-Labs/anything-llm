      ...
      setup(aibitat) {
        aibitat.function({
          ...
          handler: async function ({ threadId, body, replyAll = false, cc, bcc, htmlBody }) {
            try {
              this.super.handlerProps.log(`Using the gmail-create-draft-reply tool.`);
              ...
              if (this.super.requestToolApproval) {
                const approval = await this.super.requestToolApproval({
                  ...
                  description: `Create Gmail draft reply to thread "${threadId}"${replyAll ? " (reply all)" : ""}`,
                });
              ...
              this.super.introspect(
                `${this.caller}: Creating draft reply to thread ${threadId}${replyAll ? " (reply all)" : ""}`
              );
              ...
              const result = await gmailLib.createDraftReply(threadId, body, replyAll, options);
              ...
              this.super.handlerProps.log(`gmail-create-draft-reply error: ${e.message}`);
