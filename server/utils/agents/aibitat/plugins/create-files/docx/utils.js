...
function createCoverPageSection(docx, options) {
  ...
  if (logoBuffer) {
    ...
    try {
      ...
      const dimensions = getImageSize(logoBuffer);
      if (dimensions.width && dimensions.height) {
...
function createRunningFooter(docx, logoBuffer, theme, logoType = "png") {
  ...
  if (logoBuffer) {
    ...
    brandingCellChildren.push(new TextRun({ text: "  " }));
  }
  ...
  const brandingCell = new Paragraph({
    alignment: AlignmentType.RIGHT,
    children: brandingCellChildren,
  });
