/**
 * SWARMSY Desktop Diagnostics — centralized reason-code → diagnostic mapping.
 *
 * Safety rules enforced by this module:
 *   - No secrets, auth tokens, API keys, session values, or server DB paths are
 *     ever included in diagnostic output.
 *   - No Hosted/Admin-only data is surfaced here.
...
/**
 * All known diagnostic reason codes with their metadata.
 * Keys mirror the reason codes documented in the problem statement so that
 * server-side helpers and frontend utilities share the same canonical source.
 */
const DESKTOP_DIAGNOSTIC_CATALOG = Object.freeze({
  // ── Runtime ──────────────────────────────────────────────────────────────
  runtime_missing: {
    severity: "error",
 ...
 */
function getDiagnosticForCode(code) {
 ...
 * @returns {boolean}
 */
function isKnownDiagnosticCode(code) {
 ...
 * field.  If the reason is not in the catalog the function returns null so
 * callers can decide whether to surface a generic fallback.
 *
