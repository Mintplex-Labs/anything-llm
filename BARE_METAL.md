...
2. `cd anything-llm` and run `npm install`. This will install all dependencies to run in production as well as debug the application.
...
`cd frontend && npm run build` - this will produce a `frontend/dist` folder that will be used later.
...
_note_ You should ensure that each folder runs `npm install` again to ensure packages are up to date in case any dependencies were added, changed, or removed.
...
cd $HOME/anything-llm/frontend && npm install && npm run build && cd $HOME/anything-llm
...
cd $HOME/anything-llm/collector && npm install

echo "Installing server dependencies & running migrations"
cd $HOME/anything-llm/server && npm install
