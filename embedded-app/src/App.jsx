...
function App() {
  ...
  return (
    <div className="fixed bottom-0 right-0 mb-4 mr-4 z-50">
      <div className={`transition-all duration-300 ease-in-out ${isOpen ? 'max-w-sm p-4 bg-white rounded-lg border shadow-lg' : 'w-16 h-16 rounded-full'}`}>
        {isOpen && (
          <>
            <div className='flex justify-center'>
              <img className='px-10' src={AnythingLLMLogo} alt='AnythingLLM Logo' />
            </div>
            <h1 className="text-md text-center font-semibold">Hello from Embedded App 👋</h1>
        ...
        <button
          onClick={toggleOpen}
          className={`absolute top-0 right-0 w-16 h-16 rounded-full ${isOpen ? 'bg-white' : 'bg-blue-500'}`}
