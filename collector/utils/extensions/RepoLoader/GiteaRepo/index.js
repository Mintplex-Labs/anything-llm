...
/**
 * Load in a Gitea Repo recursively or just the top level if no PAT is provided
 * @param {object} args - forwarded request body params
 * @param {import("../../../middleware/setDataSigner").ResponseWithSigner} response - Express response object with encryptionWorker
 * @returns
 */
async function loadGiteaRepo(args, response) {
  ...
  if (!repo.ready)
    return {
      success: false,
      reason: "Could not prepare Gitea repo for loading! Check URL",
    };

  console.log(
    `-- Working Gitea ${repo.author}/${repo.project}:${repo.branch} --`
  );
  ...
  console.log(`[Gitea Loader]: Found ${docs.length} source files. Saving...`);
  ...
  for (const doc of docs) {
    ...
    console.log(
      `[Gitea Loader]: Saving ${doc.metadata.source} to ${outFolder}`
    );
...
/**
 * Gets the page content from a specific source file in a given Gitea Repo, not all items in a repo.
 * @returns
 */
async function fetchGiteaFile({
  ...
  if (!repo.ready)
    return {
      success: false,
      content: null,
      reason: "Could not prepare Gitea repo for loading! Check URL or PAT.",
    };

  console.log(
    `-- Working Gitea ${repo.author}/${repo.project}:${repo.branch} file:${sourceFilePath} --`
  );
 ...
 * Gitea is self-hosted so the protocol of the instance is stored explicitly - it cannot be
 * assumed to be `https` the way the hosted providers can.
...
function generateChunkSource(repo, doc, encryptionWorker) {
  ...
  return `gitea://${repo.repo}?payload=${encryptionWorker.encrypt(
    JSON.stringify(payload)
  )}`;
}

module.exports = { loadGiteaRepo, fetchGiteaFile, generateChunkSource };
