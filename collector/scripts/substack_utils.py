...
def fetch_all_publications(subdomain):
    ...
    if os.path.exists(file_path):
        with open(file_path, "r") as file:
            print(
                f"Returning cached data for substack {subdomain}.substack.com. If you do not wish to use stored data then delete the file for this newsletter to allow refetching."
            )
    ...
    with open(file_path, "w+", encoding="utf-8") as json_file:
        ...
        print(
            f"{len(publications)} publications found for author {subdomain}.substack.com. Saved to substack-logs/channel-{subdomain}.json"
        )
...
def only_valid_publications(publications=[]):
    ...
    for publication in publications:
        ...
        if (is_paid and publication.get("should_send_free_preview") != True) or publication.get(
            "type"
        ) != "newsletter":
            continue
