...
def get_channel_id(channel_link):
    if "@" in channel_link:
        ...
        if response.ok == False:
            print(
                "Handle => ChannelId mapping endpoint is too slow - use regular youtube.com/channel URL"
            )
            return None
...
def fetch_channel_video_information(channel_id, windowSize=50):
    ...
    if os.path.exists(file_path):
        with open(file_path, "r") as file:
            print(
                f"Returning cached data for channel {channel_id}. If you do not wish to use stored data then delete the file for this channel to allow refetching."
            )
    ...
    with open(file_path, "w+", encoding="utf-8") as json_file:
        ...
        print(
            f"{len(items)} videos found for channel {data.get('channelTitle')}. Saved to channel-logs/channel-{channel_id}.json"
        )
