...
async function processSingleFile(targetFilename) {
  ...
  if (RESERVED_FILES.includes(targetFilename)) return { success: false, reason: 'Filename is a reserved filename and cannot be processed.' }
  if (!fs.existsSync(fullFilePath)) return { success: false, reason: 'File does not exist in upload directory.' }

  const fileExtension = path.extname(fullFilePath).toLowerCase();
  if (!Object.keys(SUPPORTED_FILETYPE_CONVERTERS).includes(fileExtension)) {
    trashFile(fullFilePath);
    return { success: false, reason: `File extension ${fileExtension} not supported for parsing.` }
  }
