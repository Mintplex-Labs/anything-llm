...
export default function SuggestedChatMessages({ slug }) {
  ...
  if (loading)
    return (
      <div className="flex flex-col">
        ...
        <div className="text-white light:text-theme-text-primary text-opacity-60 text-sm font-medium mt-6">
          <PreLoader size="4" />
        </div>
      </div>
    );
  return (
    <div className="w-full mt-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-white/60 light:text-theme-text-secondary text-xs mt-2 w-full justify-center max-w-[600px]">
        {suggestedMessages.map((suggestion, index) => (
          <div key={index} className="relative w-full">
            <button
              className="transition-all duration-300 absolute z-10 text-neutral-700 bg-white rounded-full hover:bg-zinc-600 hover:border-zinc-600 hover:text-white light:hover:text-theme-text-primary border-transparent border shadow-lg ml-2"
              style={{
                top: -8,
                left: 265,
              }}
        ...
        <div className="flex flex-col gap-y-4 mr-2 mt-8">
          <div className="w-1/2">
            ...
            <input
              ...
              className="border-none bg-theme-settings-input-bg text-white light:text-theme-text-primary placeholder:text-white/20 text-sm rounded-lg focus:outline-primary-button active:outline-primary-button outline-none block p-2.5 w-full"
      ...
      {suggestedMessages.length < 4 && (
        <button
          type="button"
          onClick={addMessage}
          className="flex gap-x-2 items-center justify-center mt-6 text-white light:text-theme-text-primary text-sm hover:text-sky-400 transition-all duration-300"
        >
      ...
      {hasChanges && (
        <div className="flex justify-start py-6">
          <button
            type="button"
            className="transition-all duration-300 border border-slate-200 px-4 py-2 rounded-lg text-white light:text-theme-text-primary text-sm items-center flex gap-x-2 hover:bg-slate-200 hover:text-slate-800 focus:ring-gray-800"
