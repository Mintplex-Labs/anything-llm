    ...
    <FeatureLayout>
      <div className="flex-1 flex gap-x-6 p-4 mt-10">
        ...
        <div className="flex flex-col gap-y-[18px]">
          ...
          {/* Feature list */}
          <FeatureList
            features={configurableFeatures(t)}
            ...
            <div className="bg-[#303237] text-white rounded-xl flex-1 p-4">
              {selectedFeature ? (
                <SelectedFeatureComponent
                  feature={configurableFeatures(t)[selectedFeature]}
                ...
                <div className="flex flex-col items-center justify-center h-full text-white/60">
                  ...
                  <p className="font-medium">
                    {t("experimentalFeatures.selectFeaturePrompt")}
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="flex items-center gap-x-2">
            <div className="text-sm text-white/60 font-medium">
              {activeFeatures.includes(settings.key)
                ? t("experimentalFeatures.featureList.on")
                : t("experimentalFeatures.featureList.off")}
            </div>
...
function FeatureVerification({ children }) {
  ...
  if (
    ...
    function acceptTos(e) {
      ...
      showToast(t("featureVerification.successMessage"), "success");
      setTimeout(() => {
          ...
          >
            <div className="relative bg-main-gradient rounded-lg shadow">
                ...
                <div className="w-full flex flex-col gap-y-4 text-white">
                  <p
                    dangerouslySetInnerHTML={{
                      __html: t("featureVerification.description"),
                    }}
                  ></p>

                  <div>
                    <p>{t("featureVerification.useWarning.title")}</p>
                    <ul
                      className="list-disc ml-6 text-sm font-mono"
                      dangerouslySetInnerHTML={{
                        __html: t("featureVerification.useWarning.warnings"),
                      }}
                    ></ul>
                  </div>

                  <div>
                    <p>{t("featureVerification.conditions.title")}</p>
                    <ul
                      className="list-disc ml-6 text-sm font-mono"
                      dangerouslySetInnerHTML={{
                        __html: t("featureVerification.conditions.warnings"),
                      }}
                    ></ul>
                  </div>
                  ...
                  <p
                    dangerouslySetInnerHTML={{
                      __html: t("featureVerification.accessInfo"),
                    }}
                  ></p>
                </div>
              </div>
              <div className="flex w-full justify-between items-center p-6 space-x-2 border-t rounded-b border-gray-500/50">
                ...
                >
                  {t("featureVerification.buttons.reject")}
                </a>
                <button
                  type="submit"
                  className="transition-all duration-300 border border-slate-200 px-4 py-2 rounded-lg text-white text-sm items-center flex gap-x-2 hover:bg-slate-200 hover:text-slate-800 focus:ring-gray-800"
                >
                  {t("featureVerification.buttons.accept")}
                </button>
              </div>
            </div>
          </form>
