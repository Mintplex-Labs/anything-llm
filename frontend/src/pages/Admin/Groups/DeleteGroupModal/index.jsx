...
export default function DeleteGroupModal({ closeModal, fetchData, _group }) {
  const [error, setError] = useState(null);
  const handleDelete = async (e) => {
    setError(null);
    e.preventDefault();
    const { error, success } = await Admin.deleteGroup(_group?.id);
    if (success) {
      fetchData();
      closeModal();
      showToast("Group deleted successfully", "success", { clear: true });
    }
    setError(error);
  };
    ...
    <div className="relative w-[650px] max-w-2xl max-h-full">
      <div className="relative bg-theme-bg-secondary rounded-lg shadow border-2 border-gray-500/50 custom-theme-bg-tertiary">
        ...
        <div className="p-6 text-white custom-text-secondary">
          <p>
            Deleting a group will delete all its association with workspaces.
            <br />
            <br />
