      ...
      <div className="p-2">
        <div className="flex flex-col gap-y-[18px] max-w-[500px]">
          <div className="flex items-center gap-x-2">
            <Plug size={24} weight="bold" className="text-white light:text-theme-text-primary" />
            <label htmlFor="name" className="text-white light:text-theme-text-primary text-md font-bold">
          ...
          <p className="text-white light:text-theme-text-primary text-opacity-60 text-xs font-medium py-1.5">
            ...
            <a
              ...
              className="text-white light:text-theme-text-primary hover:underline"
            >
              {config.author}
            </a>
          </p>
          ...
          {hasSetupArgs ? (
            <div className="flex flex-col gap-y-2">
              {Object.entries(config.setup_args).map(([key, props]) => (
                <div key={key} className="flex flex-col gap-y-1">
                  <label htmlFor={key} className="text-white light:text-theme-text-primary text-sm font-bold">
                    {key}
                  </label>
                  <input
                    ...
                    className="border-solid bg-transparent border border-white light:border-black rounded-md p-2 text-white light:text-theme-text-primary text-sm"
                  />
                  <p className="text-white light:text-theme-text-primary text-opacity-60 text-xs font-medium py-1.5">
                    {props?.input?.hint}
                  </p>
            ...
            <p className="text-white light:text-theme-text-primary text-opacity-60 text-sm font-medium py-1.5">
  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        ...
        className="p-1.5 rounded-lg text-white light:text-theme-text-primary hover:bg-theme-action-menu-item-hover transition-colors duration-300"
      >
        <Gear className="h-5 w-5" weight="bold" />
      </button>
      {open && (
        <div className="absolute w-[100px] -top-1 left-7 mt-1 border-[1.5px] border-white/40 rounded-lg bg-theme-action-menu-bg flex flex-col shadow-[0_4px_14px_rgba(0,0,0,0.25)] text-white light:text-theme-text-primary z-99 md:z-10">
          <button
            type="button"
            onClick={deleteSkill}
