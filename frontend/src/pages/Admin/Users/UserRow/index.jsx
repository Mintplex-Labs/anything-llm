...
export default function UserRow({ currUser, user }) {
  ...
  const handleSuspend = async () => {
    if (
      !window.confirm(
        `Are you sure you want to suspend ${user.username}?\nAfter you do this they will be logged out and unable to log back into this instance of OneNew until unsuspended by an admin.`
      )
    )
      return false;
  ...
  const handleDelete = async () => {
    if (
      !window.confirm(
        `Are you sure you want to delete ${user.username}?\nAfter you do this they will be logged out and unable to use this instance of OneNew.\n\nThis action is irreversible.`
      )
    )
      return false;
      ...
      >
        ...
        <td className="px-6 flex items-center gap-x-6 h-full mt-2">
          {canModify && (
            <button
              onClick={openModal}
              className="text-xs font-medium text-white/80 light:text-foreground rounded-lg hover:text-white hover:light:text-gray-500 px-2 py-1 hover:bg-card hover:bg-opacity-10"
            >
              Edit
            </button>
          )}
          {currUser?.id !== user.id && canModify && (
            <>
              <button
                ...
                className="text-xs font-medium text-white/80 light:text-foreground hover:light:text-orange-500 hover:text-orange-300 rounded-lg px-2 py-1 hover:bg-card hover:light:bg-orange-50 hover:bg-opacity-10"
              >
