...
export default function UserItems({ connectionKey }) {
  ...
  return (
    <div className="flex flex-col gap-y-8">
      ...
      <div className="w-full flex flex-col gap-y-1 pb-6 border-white light:border-theme-modal-border border-b-2 border-opacity-10">
        ...
        <div className="flex flex-col gap-4 mt-4">
          {Object.keys(createdByMe).map((type) => {
            ...
            return (
              <div key={type} className="rounded-lg w-full">
                <h3 className="text-white light:text-theme-text-primary capitalize font-medium mb-3">
                  {readableType(type)}
                </h3>
          ...
          {!hasItems(createdByMe) && (
            <p className="text-white/60 light:text-theme-text-secondary text-xs text-center mt-4">
        ...
        <div className="flex flex-col gap-4 mt-4">
          {teamItems.map((team) => (
            <div key={team.teamId} className="flex flex-col gap-y-4">
              <h3 className="text-white light:text-theme-text-primary text-sm font-medium">
                {team.teamName}
              </h3>
