        ...
        <div className="flex flex-col h-full w-full items-center justify-center">
          <div className="flex flex-col items-center w-full max-w-[650px] px-4">
            ...
            <div className="flex flex-wrap justify-center gap-2 mt-6">
              <QuickActionButton label="Create an Agent" />
              <QuickActionButton label="Record a Meeting" />
              <QuickActionButton label="Upload a Document" />
            </div>
          </div>
        </div>
      </DndUploaderContext.Provider>
...
function QuickActionButton({ label, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="px-4 py-2 rounded-full border border-white/20 text-white/80 text-sm hover:bg-white/5 transition-colors"
    >
      {label}
    </button>
  );
