...
function FeatureCard({
  ...
  return (
    <div className="card-hemp flex flex-col justify-between gap-y-4">
      <div className="flex flex-col gap-y-2">
        <h2 className="text-hemp-text font-semibold flex items-center gap-x-2">
          {title}
        </h2>
        <p className="text-hemp-earth text-sm">{description}</p>
      </div>
      <div className="flex flex-col gap-y-[10px]">
        <button
          onClick={onPrimaryAction}
          className="btn-hemp-outline w-full h-[36px] text-sm font-medium flex items-center justify-center gap-x-2.5"
        >
          {primaryAction}
        </button>
        {secondaryAction && (
          <div className="relative w-full">
            {isNew && (
              <div className="absolute left-3 top-1/2 -translate-y-1/2 px-2 font-semibold rounded-md text-[10px] text-white bg-hemp-primary">
                New
              </div>
            )}
            <button
              onClick={onSecondaryAction}
              className="btn-hemp-secondary w-full h-[36px] text-sm font-medium flex items-center justify-center"
            >
              {secondaryAction}
            </button>
          </div>
