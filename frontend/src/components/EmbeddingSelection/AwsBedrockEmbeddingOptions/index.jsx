...
import { AWS_REGIONS } from "../../LLMSelection/AwsBedrockLLMOptions/regions";
import { useState } from "react";

export default function AwsBedrockEmbeddingOptions({ settings }) {
  const [connectionMethod, setConnectionMethod] = useState(
    settings?.AwsBedrockEmbeddingConnectionMethod ?? "iam"
  );
  ...
  return (
    <div className="w-full flex flex-col">
      ...
      <div className="flex flex-col gap-y-2 mb-2">
        <input
          type="hidden"
          name="AwsBedrockEmbeddingConnectionMethod"
      ...
      <div className="w-full flex items-center gap-[36px] my-1.5">
        {["iam", "sessionToken"].includes(connectionMethod) && (
          <>
            ...
            <div className="flex flex-col w-60">
              ...
              <input
                type="password"
                name="AwsBedrockEmbeddingAccessKey"
                className="border-none bg-theme-settings-input-bg text-white placeholder:text-theme-settings-input-placeholder text-sm rounded-lg focus:outline-primary-button active:outline-primary-button outline-none block w-full p-2.5"
                placeholder="AWS Bedrock IAM User Access Key"
                defaultValue={
                  settings?.AwsBedrockEmbeddingAccessKey ? "*".repeat(20) : ""
                }
                required={true}
                autoComplete="off"
                spellCheck={false}
        ...
        {connectionMethod === "sessionToken" && (
          <div className="flex flex-col w-60">
            ...
            <input
              type="password"
              name="AwsBedrockEmbeddingSessionToken"
              className="border-none bg-theme-settings-input-bg text-theme-text-primary placeholder:text-theme-settings-input-placeholder text-sm rounded-lg focus:outline-primary-button active:outline-primary-button outline-none block w-full p-2.5"
              placeholder="AWS Bedrock Session Token"
              defaultValue={
                settings?.AwsBedrockEmbeddingSessionToken ? "*".repeat(20) : ""
              }
              required={true}
              autoComplete="off"
              spellCheck={false}
        ...
        <div className="flex flex-col w-60">
          ...
          <select
            name="AwsBedrockEmbeddingRegion"
            defaultValue={settings?.AwsBedrockEmbeddingRegion || "us-west-2"}
            required={true}
            className="border-none bg-theme-settings-input-bg text-white placeholder:text-theme-settings-input-placeholder text-sm rounded-lg focus:outline-primary-button active:outline-primary-button outline-none block w-full p-2.5"
          >
        ...
        <>
          <div className="flex flex-col w-60">
            ...
            <input
              ...
              className="border-none bg-theme-settings-input-bg text-white placeholder:text-theme-settings-input-placeholder text-sm rounded-lg focus:outline-primary-button active:outline-primary-button outline-none block w-full p-2.5"
              placeholder="Model id eg: amazon.titan-embed-text-v2:0"
          ...
          <div className="flex flex-col w-60">
            <label className="text-white text-sm font-semibold block mb-3">
              Model Max Chunk Length
            </label>
            <input
              type="number"
              name="EmbeddingModelMaxChunkLength"
              className="border-none bg-theme-settings-input-bg text-white placeholder:text-theme-settings-input-placeholder text-sm rounded-lg focus:outline-primary-button active:outline-primary-button outline-none block w-full p-2.5"
              placeholder="Chunk length (eg: 8192)"
              min={1}
              onScroll={(e) => e.target.blur()}
              defaultValue={settings?.EmbeddingModelMaxChunkLength || 8000}
              required={true}
              autoComplete="off"
            />
          </div>
