  ...
  return (
    <div className="flex gap-[36px] mt-1.5">
      ...
      {!settings?.credentialsOnly && (
        <JanAiModelSelection settings={settings} apiKey={apiKey} />
      )}
    </div>
  );
}

function JanAiModelSelection({ apiKey, settings }) {
  ...
  useEffect(() => {
    async function findCustomModels() {
      ...
      try {
        setLoading(true);
        const { models } = await System.customModels("janai", apiKey);
        setModels(models || []);
      } catch (error) {
      ...
      >
        {models.map((model) => (
          <option
            key={model.id}
            value={model.id}
            selected={settings?.JanAiModelPref === model.id}
          >
            {model.id}
          </option>
        ))}
