...
export default function EmpirioLabsOptions({ settings }) {
  return (
      ...
      <div className="w-full flex items-start gap-[36px] mt-1.5">
        <div className="flex flex-col w-60">
          ...
          <input
            ...
            placeholder="EmpirioLabs API Key"
            defaultValue={settings?.EmpirioLabsApiKey ? "*".repeat(20) : ""}
            required={true}
            autoComplete="off"
            spellCheck={false}
          />
        ...
        {!settings?.credentialsOnly && (
          <EmpirioLabsModelSelection settings={settings} />
        )}
      </div>
    ...
    <div className="flex flex-col gap-y-4">
      ...
      <div hidden={!showAdvancedControls}>
        <div className="flex flex-col w-60">
          ...
          <input
            type="number"
            name="EmpirioLabsLLMTimeout"
            className="border-none bg-theme-settings-input-bg text-theme-text-primary placeholder:text-theme-settings-input-placeholder text-sm rounded-lg focus:outline-primary-button active:outline-primary-button outline-none block w-full p-2.5"
            placeholder="Timeout value between token responses to auto-timeout the stream"
            defaultValue={settings?.EmpirioLabsLLMTimeout ?? 3_000}
            autoComplete="off"
...
function EmpirioLabsModelSelection({ settings }) {
  ...
  useEffect(() => {
    async function findCustomModels() {
      setLoading(true);
      const { models: fetched = [] } = await System.customModels("empiriolabs");
      if (fetched?.length > 0) {
  ...
  return (
    <div className="flex flex-col w-60">
      ...
      <input
        type="text"
        name="EmpirioLabsModelPref"
        list="empiriolabs-models-list"
        required
        className="border-none bg-theme-settings-input-bg text-theme-text-primary placeholder:text-theme-settings-input-placeholder text-sm rounded-lg block w-full p-2.5"
        placeholder="Type or select a model"
        defaultValue={settings?.EmpirioLabsModelPref || ""}
        autoComplete="off"
        spellCheck={false}
      />
      <datalist id="empiriolabs-models-list">
        {models.map((model) => (
