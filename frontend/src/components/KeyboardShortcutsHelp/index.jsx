    ...
    <div className="fixed inset-0 z-50 overflow-auto bg-black bg-opacity-50 flex items-center justify-center">
      <div className="relative bg-theme-bg-secondary rounded-lg p-6 max-w-2xl w-full mx-4">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold text-white light:text-theme-text-primary">
            {t("keyboard-shortcuts.title")}
          </h2>
          <button
            onClick={() => setIsOpen(false)}
            className="text-white light:text-theme-text-primary hover:text-gray-300 light:text-theme-text-secondary"
            aria-label="Close"
          >
            <X size={24} />
          </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {Object.entries(SHORTCUTS).map(([key, shortcut]) => (
            ...
            >
              <span className="text-white light:text-theme-text-primary">
                {t(`keyboard-shortcuts.shortcuts.${shortcut.translationKey}`)}
              </span>
              <kbd className="px-2 py-1 bg-theme-bg-secondary text-white light:text-theme-text-primary rounded border border-gray-600 light:border-gray-300">
