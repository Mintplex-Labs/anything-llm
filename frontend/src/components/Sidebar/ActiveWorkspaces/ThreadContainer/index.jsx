...
function DeleteAllThreadButton({ ctrlPressed, threads, onDelete }) {
  ...
  return (
    ...
    >
      <div className="flex w-full gap-x-2 items-center pl-4">
        <div className="bg-theme-bg-container p-2 rounded-lg h-[24px] w-[24px] flex items-center justify-center">
          <Trash
            weight="bold"
            size={14}
            className="shrink-0 text-theme-text-primary group-hover:text-red-400"
          />
        </div>
        <p className="text-theme-text-primary text-left text-sm group-hover:text-red-400">
          Delete Selected
        </p>
      </div>
    </button>
