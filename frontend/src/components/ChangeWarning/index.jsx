    ...
    <div className="w-full max-w-2xl bg-theme-bg-secondary rounded-lg shadow border-2 border-theme-modal-border overflow-hidden z-9999">
      ...
      >
        <div className="py-7 px-9 space-y-2 flex-col">
          <p className="text-white light:text-theme-text-primary">
            {warningText.split("\\n").map((line, index) => (
              <span key={index}>
                {line}
                <br />
      ...
      <div className="flex w-full justify-end items-center p-6 space-x-2 border-t border-theme-modal-border rounded-b">
        <button
          onClick={onClose}
          type="button"
          className="transition-all duration-300 bg-transparent text-white light:text-theme-text-primary hover:opacity-60 px-4 py-2 rounded-lg text-sm border-none"
        >
          Cancel
        </button>
        <button
          onClick={onConfirm}
          type="submit"
          className="transition-all duration-300 bg-red-500 light:text-white text-white light:text-theme-text-primary hover:opacity-60 px-4 py-2 rounded-lg text-sm border-none"
        >
          Confirm
        </button>
      </div>
