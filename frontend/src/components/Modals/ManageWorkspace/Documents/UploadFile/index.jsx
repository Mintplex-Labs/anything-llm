  ...
  return (
    <div>
      ...
      >
        ...
        {ready === false ? (
          ...
          <div className="flex flex-col items-center justify-center">
            <CloudArrowUp className="w-8 h-8 text-white/80 light:invert" />
            <div className="text-white text-opacity-80 text-sm font-semibold py-1">
              {t('common.upload.click-upload')}
            </div>
            <div className="text-white text-opacity-60 text-xs font-medium py-1">
              {t('common.upload.file-types')}
            </div>
          </div>
        ) : (
      ...
      <div className="text-center text-white text-opacity-50 text-xs font-medium w-[560px] py-2">
        {t('common.upload.or-submit-link')}
      </div>
      <form onSubmit={handleSendLink} className="flex gap-x-2">
        <input
          ...
          placeholder={t('common.upload.placeholder-link')}
          autoComplete="off"
        />
        <button
          disabled={fetchingUrl}
        ...
        >
          {fetchingUrl ? t('common.upload.fetching') : t('common.upload.fetch-website')}
        </button>
      </form>
      <div className="mt-6 text-center text-white text-opacity-80 text-xs font-medium w-[560px]">
        {t('common.upload.privacy-notice')}
      </div>
    </div>
  );
}
