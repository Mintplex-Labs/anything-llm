...
export default function GiteaOptions() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data, error } = await System.dataConnectors.gitea.collect({
        repo: form.get("repo"),
                  ...
                  <label className="text-white font-bold text-sm flex gap-x-2 items-center">
                    ...
                    <p className="text-xs font-light flex items-center">
                      <span className="text-theme-text-secondary">
                        {t("connectors.gitea.optional")}
                      </span>
                      <PATTooltip accessToken={accessToken} />
                    </p>
                  </label>
                  <p className="text-xs font-normal text-theme-text-secondary">
                    {t("connectors.gitea.token_explained")}
                  </p>
                </div>
                <input
                  type="text"
          ...
          <div className="flex flex-col gap-y-2 w-full pr-10">
            ...
            {loading && (
              <p className="text-xs text-white/50">
                {t("connectors.gitea.task_explained")}
              </p>
            )}
          </div>
        </form>
...
function GiteaBranchSelection({ repo, accessToken }) {
  const { t } = useTranslation();
  ...
  useEffect(() => {
    async function fetchAllBranches() {
      ...
      setLoading(true);
      const { branches } = await System.dataConnectors.gitea.branches({
        repo,
        accessToken,
      });
      ...
      <div className="flex flex-col w-60">
        ...
        >
          <option disabled={true} selected={true}>
            {t("connectors.gitea.branch_loading")}
          </option>
        </select>
      </div>
    );
        ...
        <p className="text-sm">
          <span
            dangerouslySetInnerHTML={{
              __html: t("connectors.gitea.token_information"),
            }}
          />
        </p>
      </div>
    ...
    <>
      ...
      >
        <p className="text-sm">
          ...
          >
            {t("connectors.gitea.token_explained_link1")}
          </a>
          {t("connectors.gitea.token_explained_end")}
        </p>
      </Tooltip>
    </>
  );
