...
const TRANSLATIONS = {
  ...
  "chat": {
    ...
    "mode": {
      ...
      "query": {
        ...
        "description": "該当する情報が見つかった場合、回答を<b>のみ</b>提供します。ツールを使用するには、@agentコマンドを使用する必要があります。"
      },
      "automatic": {
        "title": "自動車",
        "description": "ネイティブなツール呼び出しをサポートしている場合、モデルとプロバイダーが自動的にツールを使用します。<br />ネイティブなツール呼び出しがサポートされていない場合は、@agentコマンドを使用してツールを使用する必要があります。"
      }
    },
    "history": {
      "title": "チャット履歴",
  ...
  "customization": {
    ...
    "items": {
      ...
      "chat-message-alignment": {
        ...
        "description": "チャットインターフェースを使用する場合、メッセージの配置モードを選択してください。"
      },
      "display-language": {
        "title": "表示言語",
      ...
      "welcome-messages": {
        "title": "ようこそ",
        "description": "ユーザーに表示されるウェルカムメッセージをカスタマイズできます。これらのメッセージは、管理者以外のユーザーのみが表示します。",
        "new": "新しい",
        "system": "システム",
  ...
  "community_hub": {
    "publish": {
      ...
      "agent_flow": {
        "public_description": "一般の利用者は、これらの流れをすべて把握することができます。",
        "private_description": "あなただけが確認できるプライベートな取引フローのみが表示されます。",
        "success_title": "成功！",
      ...
      "slash_command": {
        ...
        "command_label": "命令",
        "command_description": "これは、ユーザーがこのプリセットを起動するために入力するスラッシュコマンドです。",
