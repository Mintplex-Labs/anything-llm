...
const TRANSLATIONS = {
  "onboarding": {
    ...
    "userSetup": {
      ...
      "adminUsernameReq": "ユーザー名は6文字以上で、小文字の英字、数字、アンダースコア、ハイフンのみを含む必要があります。スペースは使用できません。",
  ...
  "welcomeMessage": {
    "part1": "AnythingLLMへようこそ。AnythingLLMはMintplex LabsによるオープンソースのAIツールで、あらゆるものをトレーニングされたチャットボットに変換し、クエリやチャットが可能です。AnythingLLMはBYOK（Bring-Your-Own-Keys）ソフトウェアであり、このソフトウェアを使用するためのサブスクリプション、料金、または費用はありません。",
    "part2": "AnythingLLMは、OpenAi、GPT-4、LangChain、PineconeDB、ChromaDBなどの強力なAI製品を簡単に統合し、手間をかけずに生産性を100倍に向上させる最も簡単な方法です。",
  ...
  "agent": {
    ...
    "skill": {
      ...
      "description": "これらのスキルでデフォルトエージェントの能力を強化できます。設定はすべてのワークスペースに適用されます。",
      "rag": {
        "title": "RAGと長期記憶",
      ...
      "save": {
        "title": "ファイルの生成と保存",
        "description": "デフォルトエージェントがファイルを生成し、ブラウザからダウンロードできるようにします。"
      },
      "web": {
        "title": "ウェブ検索と閲覧",
        "desc-start": "エージェントがウェブ検索プロバイダーに接続し、質問に答えるためにウェブ検索できるようにします。",
  ...
  "embedding": {
    ...
    "provider": {
      "title": "埋め込みプロバイダー",
      "description": "AnythingLLMのネイティブ埋め込みエンジンを使用する場合、特に設定は必要ありません。"
    }
  },
  "text": {
  ...
  "profile_settings": {
    ...
    "username": "ユーザー名",
    "username_description": "ユーザー名は小文字の英字、数字、アンダースコア、ハイフンのみ使用でき、スペースは使えません",
