...
const TRANSLATIONS = {
  ...
  llm: {
    ...
    provider: "LLM 提供商",
    searchPlaceholder: "搜索可用的 LLM 提供商",
    noneSelected: "未选择",
    selectLLM: "您需要选择一个 LLM",
    saving: "保存中...",
    saveChanges: "保存更改",
    saveSuccess: "LLM 偏好设置已成功保存。",
    saveError: "保存 LLM 设置失败：{{error}}",
    providers: {
      defaultName: "系统默认",
      defaultDescription: "为此工作区使用系统 LLM 偏好设置。",
      apiKeyLabel: "{{provider}} API 密钥",
      apiKeyPlaceholder: "{{provider}} API 密钥",
      modelLabel: "聊天模型选择",
      openai: "大多数非商业用途的标准选项。",
      azure: "在 Azure 服务上托管的 OpenAI 企业选项。",
      anthropic: "由 Anthropic 托管的友好 AI 助手。",
      gemini: "Google 最大和最强大的 AI 模型",
      huggingface: "访问 150,000 多个开源 LLM 和全球 AI 社区",
      ollama: "在您自己的机器上本地运行 LLM。",
      lmstudio: "发现、下载并运行数千个先进的 LLM，只需点击几下。",
      localai: "在您自己的机器上本地运行 LLM。",
      togetherai: "运行 Together AI 的开源模型。",
      mistral: "运行 Mistral AI 的开源模型。",
      perplexity: "运行由 Perplexity AI 托管的强大且连接到互联网的模型。",
      openrouter: "LLM 的统一接口。",
      groq: "实时 AI 应用中最快的 LLM 推理。",
      koboldcpp: "使用 koboldcpp 运行本地 LLM。",
      textgenwebui: "使用 Oobabooga 的文本生成 Web UI 运行本地 LLM。",
      cohere: "运行 Cohere 的强大 Command 模型。",
      litellm: "运行 LiteLLM 的兼容 OpenAI 的代理，支持各种 LLM。",
      genericopenai: "通过自定义配置连接到任何兼容 OpenAi 的服务",
      native: "使用下载的自定义 Llama 模型在此 TeamplGPT 实例上进行聊天。",
      safetyLabel: "安全设置",
      safety: {
        none: "无",
        blockFew: "阻止一些",
      ...
      baseUrl: "基本 URL",
      tokenContextWindow: "令牌上下文窗口",
      loadingModels: "-- 正在加载可用模型 --",
      waitingForUrl: "-- 等待 URL --",
      waitingForAPI: "-- 等待 API --",
      apiKey: "API 密钥",
      loadedModels: "您加载的模型",
    },
  },
  ...
  azure: {
    serviceEndpoint: "Azure 服务端点",
    chatDeploymentName: "聊天部署名称",
    chatDeploymentNamePlaceholder: "Azure OpenAI 聊天模型部署名称",
    chatModelTokenLimit: "聊天模型令牌限制",
    embeddingDeploymentName: "嵌入部署名称",
    embeddingDeploymentNamePlaceholder: "Azure OpenAI 嵌入模型部署名称",
  },

  huggingface: {
    inferenceEndpoint: "HuggingFace 推理端点",
    accessToken: "HuggingFace 访问令牌",
    accessTokenPlaceholder: "HuggingFace 访问令牌",
    tokenLimit: "模型令牌限制",
  },

  genericOpenAi: {
    baseUrl: "基础 URL",
    chatModelName: "聊天模型名称",
    chatModelNamePlaceholder: "用于聊天请求的模型 ID",
    tokenContextWindow: "令牌上下文窗口",
    tokenContextWindowPlaceholder: "上下文窗口限制（例如：4096）",
    maxTokens: "最大令牌数",
    maxTokensPlaceholder: "每个请求的最大令牌数（例如：1024）",
  },

  ollama: {
    maxTokens: "最大令牌数",
    maxTokensDescription: "上下文和响应的最大令牌数。",
    hideManualInput: "隐藏手动输入端点",
    showManualInput: "显示手动输入端点",
    baseUrl: "Ollama 基础 URL",
    baseUrlDescription: "输入 Ollama 运行的 URL。",
    autoDetect: "自动检测",
    enterUrlFirst: "首先输入 Ollama URL",
    model: "Ollama 模型",
    modelDescription:
      "选择您想要使用的 Ollama 模型。在输入有效的 Ollama URL 后加载模型。",
    maxChunkLength: "最大嵌入块长度",
    maxChunkLengthDescription: "嵌入的文本块的最大长度。",
  },

  lmstudio: {
    alertMessage: "LMStudio 作为您的 LLM 需要您设置要使用的嵌入服务。",
    manageEmbedding: "管理嵌入 →",
    maxTokens: "最大令牌数",
    maxTokensDescription: "上下文和响应的最大令牌数。",
    hideManualInput: "隐藏手动输入端点",
    showManualInput: "显示手动输入端点",
    baseUrl: "LM Studio 基础 URL",
    baseUrlDescription: "输入 LM Studio 运行的 URL。",
    autoDetect: "自动检测",
    enterUrlFirst: "首先输入 LM Studio URL",
    model: "LM Studio 模型",
    modelDescription:
      "选择您想要使用的 LM Studio 模型。在输入有效的 LM Studio URL 后加载模型。",
    maxChunkLength: "最大嵌入块长度",
    maxChunkLengthDescription: "嵌入的文本块的最大长度。",
  },

  localai: {
    alertMessage: "LocalAI 作为您的 LLM 需要您设置要使用的嵌入服务。",
    manageEmbedding: "管理嵌入 →",
    baseUrl: "Local AI 基础 URL",
    tokenContextWindow: "令牌上下文窗口",
    apiKey: "Local AI API 密钥",
    modelSelection: "聊天模型选择",
    optional: "可选",
  },
  ...
  nativellm: {
    experimentalWarning: "使用本地托管的 LLM 是实验性的。请谨慎使用。",
    modelSelection: "模型选择",
    waitingForModels: "-- 正在等待模型 --",
    tokenContextWindow: "令牌上下文窗口",
  },
