...
const TRANSLATIONS = {
  ...
  "chat": {
    ...
    "mode": {
      ...
      "chat": {
        ...
        "description": "将提供答案，利用LLM的通用知识和相关文档的上下文信息。您需要使用 `@agent` 命令来使用工具。"
      },
      "query": {
        "title": "查询",
        "desc-start": "将会提供答案",
        "only": "仅当",
        "desc-end": "找到文档上下文时。",
        "description": "将在找到文档上下文时提供答案 <b>仅限</b>。您需要使用 @agent 命令来使用工具。"
      },
      "automatic": {
        "title": "自动",
        "description": "如果模型和提供商支持原生工具调用，则会自动使用这些工具。<br />如果不支持原生工具调用，则需要使用 `@agent` 命令来使用工具。"
      }
    },
    "history": {
      "title": "聊天历史记录",
  ...
  "agent": {
    ...
    "skill": {
      ...
      "web": {
        "title": "实时网络搜索和浏览",
        "desc-start": "通过连接到网络搜索（搜索结果页）提供者，使你的代理能够搜索网络以回答你的问题。",
