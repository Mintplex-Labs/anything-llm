...
const TRANSLATIONS = {
  ...
  "chat": {
    ...
    "mode": {
      ...
      "chat": {
        "title": "צ'אט",
        "description": "יוכל לספק תשובות בהתבסס על הידע הכללי של ה-LLM ועל ההקשר הרלוונטי מהמסמכים. <b> ו-</b>\nתצטרכו להשתמש בפקודה @agent כדי להשתמש בכלי."
      },
      "query": {
        "title": "שאילתה",
  ...
  "agent": {
    ...
    "mcp": {
      ...
      "tool-count-warning": "שרת ה-MCP הזה כולל <b>כלי{{count}} שפעילים, אשר יצרוך מידע הקשר בכל צ'אט. </b> מומלץ לבטל את הכליות הלא רצויות כדי לחסוך במידע ההקשר.",
    ...
    "settings": {
      ...
      "intelligent-skill-selection": {
        ...
        "beta-badge": "בטא",
        "description": "אפשרו שימוש בלתי מוגבל בכלים וצמצום השימוש בטוקנים עד 80% לכל שאילתה – ה-AnythingLLM בוחר באופן אוטומטי את הכישורים המתאימים ביותר לכל בקשה.",
        "max-tools": {
          "title": "כלים של מקס",
  ...
  "api": {
    ...
    "modal": {
      ...
      "close": "סגירה",
      "create": "צור מפתח API",
      "helper": "לאחר יצירתו, ניתן להשתמש במפתח ה-API כדי לגשת למופע AnythingLLM זה ולהגדיר אותו באופן תכנותי.",
      "name": {
        "label": "שם",
  ...
  "chat_window": {
    ...
    "agent_invocation": {
      ...
      "clarifying_question_header": "למתווך יש צורך במידע נוסף",
      "clarifying_skip": "השליח יחליט",
      "clarifying_submit": "הגש",
      "clarifying_skipped": "אתם מאפשרים לגורם המוסמך לקבל את ההחלטה.",
      "clarifying_timeout": "לא התקבלה תגובה בזמן.",
      "clarifying_pagination": "{{current}} של {{total}}",
      "clarifying_prev_aria": "השאלה הקודמת",
      "clarifying_next_aria": "השאלה הבאה",
      "clarifying_close_aria": "סגור וקפוץ",
  ...
  "telegram": {
    "title": "בוט של טלגרם",
    "description": "חברו את ההתקנה של AnythingLLM ל-Telegram, כך שתוכלו לתקשר עם סביבות העבודה שלכם ממכשיר כלשהו.",
    "setup": {
      "step1": {
        "title": "שלב 1: צרו את הבוט שלכם ב-Telegram",
        "description": "פתח את <code> ב-Telegram, שלח </code> לכתובת <code>@BotFather, עקוב אחר ההוראות, והעתק את מזהה ה-API.",
