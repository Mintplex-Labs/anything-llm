...
const TRANSLATIONS = {
  "onboarding": {
    ...
    "llm": {
      "title": "LLM 偏好",
      "description": "AnythingLLM 可搭配多種 LLM 提供者使用。這項服務會負責處理對話。"
    },
    "userSetup": {
      "title": "使用者設定",
      "description": "設定使用者偏好。",
      ...
      "teamHint": "預設只有您擁有管理員權限。完成初始設定後，即可建立帳號並邀請其他人成為使用者或管理員。請勿遺失密碼，因為只有管理員能重設密碼。"
    },
    "data": {
      "title": "資料處理與隱私",
    ...
    "survey": {
      ...
      "commentPlaceholder": "例如 Reddit、Twitter、GitHub、YouTube 等，告訴我們您是怎麼找到 AnythingLLM 的！",
      "skip": "略過問卷",
      "thankYou": "感謝您的回饋！"
    }
  },
  "common": {
    ...
    "no": "否",
    "search": "搜尋",
    "username_requirements": "使用者名稱必須為 2-32 個字元，以小寫字母開頭，且只能包含小寫字母、數字、底線、連字號和句點。",
    "on": "關於",
    "none": "沒有",
    "stopped": "停止",
    "loading": "載入",
  ...
  "general": {
    ...
    "delete": {
      ...
      "confirm-end": "工作區。這會移除向量資料庫中的所有向量嵌入內容。\n\n原始來源檔案不會受影響。此動作無法復原。"
    }
  },
  "chat": {
    "llm": {
      "title": "工作區 LLM 提供者",
      "description": "這個工作區要使用的 LLM 提供者與模型。預設會沿用系統層級的 LLM 提供者與設定。",
      "search": "搜尋所有 LLM 提供者"
    },
    "model": {
      "title": "工作區對話模型",
      "description": "這個工作區要使用的對話模型。若留空，會沿用系統層級的 LLM 偏好設定。"
    },
    "mode": {
      "title": "對話模式",
      "chat": {
        "title": "對話",
        "description": "將提供答案，並利用 LLM 的通用知識和相關文件內容。您需要使用 @agent 命令來使用工具。"
      },
      "query": {
        "title": "查詢",
        "description": "將提供答案，但僅在確認文件內容時.<b>您需要使用 `@agent` 命令來使用工具。"
      },
      "automatic": {
        "title": "自動",
        "description": "如果模型和供應商支援原生工具調用，則系統會自動使用這些工具。<br />如果原生工具調用不受支援，您需要使用 `@agent` 命令來使用工具。"
      }
    },
    "history": {
      "title": "對話紀錄",
    ...
    "prompt": {
      "title": "系統提示詞",
      "description": "這是此工作區會使用的提示詞，用來定義 AI 產生回應時的脈絡與指示。請提供經過仔細設計的提示詞，讓 AI 能產生相關且準確的回應。",
      "history": {
        "title": "系統提示詞歷史記錄",
        "clearAll": "清除全部",
    ...
    "refusal": {
      ...
      "tooltip-description": "目前處於查詢模式，這個模式只會使用文件中的資訊。若想進行更彈性的對話，請切換到對話模式；或點選這裡前往文件，進一步了解對話模式。"
    },
    "temperature": {
      "title": "LLM 溫度",
  ...
  "vector-workspace": {
    ...
    "doc": {
      "title": "文件相似度門檻",
      "description": "來源至少要達到這個相似度分數，才會被視為與對話相關。數值越高，代表來源必須越接近對話內容。",
      "zero": "不限制",
      "low": "低（相似度分數 ≥ .25）",
      "medium": "中（相似度分數 ≥ .50）",
      "high": "高（相似度分數 ≥ .75）"
    ...
    "reset": {
      "reset": "重設向量資料庫",
      "resetting": "正在清除向量...",
      "confirm": "您即將重設這個工作區的向量資料庫。這會移除目前所有已嵌入的向量。\n\n原始來源檔案不會受影響。此動作無法復原。",
      "error": "無法重設工作區向量資料庫！",
      "success": "工作區向量資料庫已重設！"
    }
  },
  "agent": {
    "performance-warning": "對於未明確支援工具呼叫的 LLM，其效能高度仰賴模型本身的能力與準確度。部分功能可能受限，甚至無法使用。",
    "provider": {
    ...
    "skill": {
      "rag": {
        "title": "RAG 與長期記憶體",
        "description": "讓智慧代理人可運用本機文件回答問題，也能要求智慧代理人「記住」特定內容片段，以供長期記憶擷取。"
      },
      "view": {
        "title": "檢視並摘要文件",
      ...
      "web": {
        "title": "網頁搜尋",
        "description": "透過連接網頁搜尋 (SERP) 提供者，讓智慧代理人能搜尋網路並回答問題。"
      },
      "sql": {
        "title": "SQL 連接器",
        "description": "讓您的智慧代理人能夠利用 SQL 查詢來回答您的問題，只需連接到不同的 SQL 資料庫提供者即可。"
      },
      "default_skill": "這項技能預設為啟用；若不希望智慧代理人使用，也可以停用。",
      "filesystem": {
        "title": "檔案系統存取",
        "description": "允許您的代理程式在指定目錄中讀取、寫入、搜尋和管理檔案。支援檔案編輯、目錄導航和內容搜尋功能。",
        "learnMore": "了解更多關於如何運用這項技能的資訊",
        "configuration": "設定",
        "readActions": "閱讀行動",
        "writeActions": "撰寫動作",
        "warning": "訪問檔案系統可能存在風險，因為它可能會修改或刪除檔案。在啟用之前，請務必查閱相關<a>文件</a>。",
        "skills": {
          "read-text-file": {
            "title": "開啟檔案",
      ...
      "createFiles": {
        "title": "文件建立",
        "description": "允許您的代理創建二元文件格式，例如PowerPoint簡報、Excel電子表格、Word文件和PDF文件。 文件可以直接從聊天窗口下載。",
        "configuration": "可用的文件類型",
        "skills": {
          "create-text-file": {
            "title": "文字檔",
            "description": "能夠創建包含任何內容和檔案擴展名（例如：.txt、.md、.json、.csv 等）的文字檔案。"
          },
          "create-pptx": {
            "title": "PowerPoint 簡報",
          ...
          "create-pdf": {
            "title": "PDF 文件",
            "description": "能夠從 Markdown 或純文字檔案中，使用基本的格式設定，創建 PDF 文件。"
          },
          "create-xlsx": {
            "title": "Excel 試算表",
      ...
      "gmail": {
        "title": "Gmail 連接器",
        "description": "讓您的代理能夠與 Gmail 互動：搜尋郵件、閱讀郵件討論、撰寫草稿、發送郵件，以及管理您的收件匣。請參閱相關文件。",
        "multiUserWarning": "由於安全考量，Gmail 整合功能在多使用者模式下無法使用。請停用多使用者模式才能使用此功能。",
      ...
      "outlook": {
        "title": "Outlook 連接器",
        "description": "讓您的代理能夠與 Microsoft Outlook 互動 - 搜尋郵件、閱讀討論串、撰寫草稿、發送郵件，以及透過 Microsoft Graph API 管理您的收件匣。 詳情請參考<a>相關文件</a>。",
        "multiUserWarning": "由於安全原因，Outlook 整合功能在多使用者模式下無法使用。請先停用多使用者模式，才能使用此功能。",
        "configuration": "視圖配置",
        "authType": "帳戶類型",
        "authTypeHelp": "選擇哪些類型的 Microsoft 帳戶可以進行驗證。「所有帳戶」支援個人帳戶和工作/學校帳戶。「僅限個人帳戶」僅支援個人 Microsoft 帳戶。「僅限組織帳戶」僅支援特定 Azure AD 租戶中的工作/學校帳戶。",
        ...
        "tenantId": "租戶編號",
        "tenantIdHelp": "您的 Azure AD 應用程式註冊的「租戶 ID」。僅用於組織內單向驗證。",
        "clientSecret": "客戶密鑰",
        "clientSecretHelp": "您的 Azure AD 應用程式註冊時所使用的客戶端秘密值",
        "configurationRequired": "請設定 Client ID 和 Client Secret，以便啟用 Outlook 相關功能。",
        "authRequired": "首先儲存您的帳戶資訊，然後再透過 Microsoft 進行驗證，以完成設定。",
      ...
      "googleCalendar": {
        "title": "Google 日曆連線",
        "description": "讓您的代理能夠與 Google 日曆互動：查看日曆、取得活動資訊、創建和更新活動，以及管理確認出席。 詳情請參考<a>文件</a>。",
        "multiUserWarning": "由於安全原因，Google 日曆的整合功能在多使用者模式下無法使用。請先停用多使用者模式，然後再使用此功能。",
        "configuration": "Google 日曆設定",
        "deploymentId": "部署 ID",
        "deploymentIdHelp": "您的 Google Apps Script 網頁應用程式的部署 ID",
        "apiKey": "API 關鍵字",
        "apiKeyHelp": "您在 Google Apps Script 部署中設定的 API 金鑰。",
        "configurationRequired": "請設定部署 ID 和 API 鑰，以啟用 Google 日曆功能。",
        "configured": "已設定",
        "searchSkills": "搜尋技巧...",
        "noSkillsFound": "沒有符合您搜尋條件的結果。",
        "categories": {
        ...
        "skills": {
          ...
          "quickAdd": {
            "title": "快速新增活動",
            "description": "從自然語言（例如：「明天下午 3 點舉行會議」）創建活動。"
          },
          "createEvent": {
            "title": "建立活動",
    ...
    "mcp": {
      ...
      "tool-count-warning": "這個 MCP 伺服器已啟用 <b> 工具，這些工具會消耗聊天中的語境 </b>。建議停用不需要的工具，以節省語境。",
      "startup-command": "啟動指令",
      "command": "指令",
      "arguments": "辯論",
      "not-running-warning": "這個 MCP 伺服器目前處於停止狀態，可能是因為已停止運作，或是啟動時出現錯誤。",
    ...
    "settings": {
      ...
      "max-tool-calls": {
        "title": "每次回應的最大工具呼叫次數",
        "description": "這設定了代理可以串聯使用的最大工具數量，以確保每次回應只會呼叫有限的工具，並避免無限循環。"
      },
      "intelligent-skill-selection": {
        "title": "智能技能選擇",
        "beta-badge": "β 版本",
        "description": "啟用無限多個工具，並將每個查詢的 token 使用量最多降低 80% — AnythingLLM 能夠自動選擇最適合的技能，以處理每一個提示。",
        "max-tools": {
          "title": "馬克斯工具",
          "description": "可選取的工具的最大數量，適用於每個查詢。我們建議將此值設定為較高的值，以適用於較大的模型。"
        }
      }
    }
  },
  ...
  "api": {
    "title": "API 金鑰",
    "description": "API 金鑰可讓持有人透過程式方式存取並管理這個 AnythingLLM 系統。",
    "link": "閱讀 API 文件",
    ...
    "modal": {
      ...
      "close": "關閉",
      "create": "建立 API 金鑰",
      "helper": "建立後，API 金鑰可用於以程式方式存取並設定這個 AnythingLLM 執行個體。",
      "name": {
        "label": "名稱",
        "placeholder": "正式環境整合",
  ...
  "llm": {
    "title": "LLM 偏好設定",
    "description": "這裡設定偏好的 LLM 對話與嵌入提供者之認證資訊與參數。請確認金鑰保持最新且正確，否則 AnythingLLM 可能無法正常運作。",
    "provider": "LLM 提供者",
    "providers": {
      "azure_openai": {
        ...
        "default": "預設",
        "reasoning": "推理",
        "model_type_tooltip": "如果您的部署使用推理模型（例如 o1、o1-mini、o3-mini 等），請將此設定設為「推理」。否則，您的對話請求可能會失敗。"
      }
    }
  },
  "transcription": {
    "title": "語音轉錄模型偏好設定",
    "description": "這裡設定偏好的語音轉錄模型提供者之認證資訊與參數。請確認金鑰保持最新且正確，否則媒體檔與音訊可能無法完成轉錄。",
    "provider": "語音轉錄提供者",
    "warn-start": "在記憶體或 CPU 資源有限的電腦上使用本機 Whisper 模型時，處理媒體檔案可能會讓 AnythingLLM 卡住。",
    "warn-recommend": "建議至少保留 2 GB 記憶體，且上傳檔案小於 10 MB。",
    "warn-end": "內建模型將會在第一次使用時自動下載。"
  },
  "embedding": {
    "title": "嵌入模型偏好設定",
    "desc-start": "使用原生不支援嵌入引擎的 LLM 時，可能需要另外提供文字嵌入的認證資訊。",
    "desc-end": "嵌入是把文字轉成向量的過程。這些認證資訊用來把檔案與提示詞轉成 AnythingLLM 可處理的格式。",
    "provider": {
      "title": "向量嵌入提供者"
    }
  },
  "text": {
    "title": "文字分割與切塊偏好設定",
    "desc-start": "有時您可能想調整新文件在寫入向量資料庫前的預設分割與切塊方式。",
    "desc-end": "只有在清楚了解文字分割的運作方式及其副作用時，才建議調整此設定。",
    "size": {
      "title": "文字區塊大小",
  ...
  "vector": {
    "title": "向量資料庫",
    "description": "這裡設定 AnythingLLM 系統運作所需的認證資訊與參數。請務必確認金鑰保持最新且正確。",
    "provider": {
      "title": "向量資料庫提供者",
  ...
  "embeddable": {
    "title": "可嵌入對話元件",
    "description": "可嵌入對話元件是綁定單一工作區、可對外公開的對話介面。您可以建立工作區，再將它發布給外部使用。",
    "create": "建立嵌入元件",
    "table": {
      "workspace": "工作區",
      "chats": "已傳送對話",
  ...
  "connectors": {
    ...
    "github": {
      ...
      "ignores": "忽略檔案",
      "git_ignore": "以 .gitignore 格式列出以忽略特定檔案。每輸入一個條目後按 Enter 鍵儲存。",
      "task_explained": "完成後，所有檔案將可供嵌入到工作區中的檔案選擇器。",
      "branch": "您希望收集檔案的分支。",
      "branch_loading": "-- 載入可用分支 --",
      "branch_explained": "您希望收集檔案的分支。",
      "token_information": "若未填寫 <b>GitHub 存取權杖</b>，此資料連接器僅能收集儲存庫的 <b>頂層</b> 檔案，因 GitHub 的公開 API 速率限制。",
    ...
    "gitlab": {
      ...
      "branch_loading": "-- 載入可用分支 --",
      "branch_explained": "您希望收集檔案的分支。",
      "token_information": "若未填寫 <b>GitLab 存取權杖</b>，此資料連接器僅能收集儲存庫的 <b>頂層</b> 檔案，因 GitLab 的公開 API 速率限制。",
    ...
    "website-depth": {
      ...
      "task_explained": "完成後，所有擷取的內容將可供嵌入到工作區中的檔案選擇器。"
    },
    "confluence": {
      "name": "Confluence",
      "description": "一鍵匯入整個 Confluence 頁面。",
      "deployment_type": "Confluence 部署類型",
      "deployment_type_explained": "確認 Confluence 環境是託管於 Atlassian 雲端，還是自行託管。",
      "base_url": "Confluence 基本網址",
      "base_url_explained": "這是您的 Confluence 空間的基本網址。",
      "space_key": "Confluence 空間金鑰",
      "space_key_explained": "這是 Confluence 環境要使用的空間金鑰，通常會以 ~ 開頭。",
      ...
      "bypass_ssl_explained": "若是使用自簽憑證的自行託管 Confluence 環境，可啟用此選項略過 SSL 憑證驗證。"
    },
    "manage": {
      "documents": "文件",
      "data-connectors": "資料連接器",
      "desktop-only": "編輯這些設定僅在桌面裝置上可用。請在桌面上開啟此頁面以繼續。",
      "dismiss": "忽略",
      "editing": "編輯中"
    },
    "directory": {
      ...
      "delete-confirmation": "您確定要刪除這些檔案和資料夾嗎？\n這將從系統中刪除這些檔案並自動從任何現有工作區中移除它們。\n此操作無法還原。",
      "removing-message": "正在刪除 {{count}} 份文件和 {{folderCount}} 個資料夾，請稍候。",
    ...
    "upload": {
      ...
      "processor-offline-desc": "目前無法上傳檔案，因為文件處理器已離線。請稍後再試。",
      ...
      "fetching": "正在擷取...",
      "fetch-website": "擷取網站",
      "privacy-notice": "這些檔案會上傳到此 AnythingLLM 系統上的文件處理器，不會傳送給或分享給第三方。"
    },
    "pinning": {
      "what_pinning": "什麼是文件釘選？",
      "pin_explained_block1": "當您在 AnythingLLM 中<b>釘選</b>文件時，系統會把整份文件的內容注入提示詞輸入區，讓 LLM 能完整理解。",
      "pin_explained_block2": "這最適合搭配<b>大上下文模型</b>，或對知識庫很重要的小型文件。",
      "pin_explained_block3": "如果 AnythingLLM 在預設情況下給不出想要的答案，釘選文件是快速提升回答品質的好方法。",
      "accept": "好的，明白了"
    },
    "watching": {
      "what_watching": "追蹤文件有何作用？",
      "watch_explained_block1": "當您在 AnythingLLM 中<b>追蹤</b>文件時，系統會<i>自動</i>定期從原始來源同步內容，並更新所有管理這份文件的工作區。",
      "watch_explained_block2": "目前這項功能只支援線上來源內容，手動上傳的文件無法使用。",
    ...
    "obsidian": {
      ...
      "vault_description": "選擇您的 Obsidian Vault 資料夾以匯入所有筆記及其連結。",
  ...
  "chat_window": {
    ...
    "agent_skills_disabled_in_session": "啟用智慧代理人工作階段時無法修改技能。請先使用 /exit 指令結束目前工作階段。",
    "start_agent_session": "開始智慧代理人工作階段",
    "use_agent_session_to_use_tools": "若要在對話中使用工具，請在提示詞開頭加上 '@agent'，即可開始智慧代理人工作階段。",
    "agent_invocation": {
      ...
      "clarifying_question_header": "經紀人需要更多資訊",
      "clarifying_skip": "請讓代理決定",
      "clarifying_submit": "提交",
      "clarifying_skipped": "您可以讓代理決定。",
      "clarifying_timeout": "未在規定時間內提交回覆。",
      "clarifying_pagination": "{{current}} 在 {{total}}",
      "clarifying_prev_aria": "上一問題",
      "clarifying_next_aria": "接下來的問題",
      "clarifying_close_aria": "關閉和跳過",
      "clarifying_other": "其他",
      "clarifying_other_placeholder": "請在此處輸入您的答案",
      "batch_progress": "{{answered}} 來自 {{total}} 的提問已獲得解答",
      "batch_skip_this": "跳過",
      "batch_submit_all": "提交所有",
      "batch_next": "下一頁",
      "answer_skipped": "[使用者已跳過]"
    },
    "custom_skills": "客製化技能",
    "agent_flows": "代理人流",
  ...
  "community_hub": {
    "publish": {
      "system_prompt": {
        ...
        "tags_label": "標籤",
        "tags_description": "標籤用來標示系統提示詞，方便搜尋。可新增多個標籤，最多 5 個，每個標籤最多 20 個字元。",
      ...
      "agent_flow": {
        ...
        "tags_label": "標籤",
        "tags_description": "標籤用來標示代理流程，方便搜尋。可新增多個標籤，最多 5 個，每個標籤最多 20 個字元。",
        "tags_placeholder": "輸入並按 Enter 鍵添加標籤",
        "visibility_label": "可見範圍",
        "submitting": "發布中...",
        "submit": "發布到社群中心",
        "privacy_note": "代理流程一律會先以私人方式上傳，以保護敏感資料。發布後可再到社群中心調整可見範圍。發布前請先確認流程中不含任何敏感或私人資訊。"
      },
      "generic": {
        "unauthenticated": {
          "title": "需要驗證",
      ...
      "slash_command": {
        ...
        "tags_label": "標籤",
        "tags_description": "標籤用來標示斜線指令，方便搜尋。可新增多個標籤，最多 5 個，每個標籤最多 20 個字元。",
        "tags_placeholder": "輸入並按 Enter 鍵添加標籤",
        "visibility_label": "可見範圍",
  ...
  "security": {
    ...
    "multiuser": {
      ...
      "enable": {
        "is-enable": "多使用者模式已啟用",
        "enable": "啟用多使用者模式",
        "description": "預設只有您具備管理員權限。身為管理員，您需要為所有新使用者或管理員建立帳號。請勿遺失密碼，因為只有管理員可以重設密碼。",
        "username": "管理員帳號使用者名稱",
        "password": "管理員帳號密碼"
      }
    },
    "password": {
      "title": "密碼保護",
      "description": "使用密碼保護 AnythingLLM 系統。若忘記此密碼，將無法復原，請務必妥善保存。",
      "password-label": "系統密碼"
    }
  },
  "home": {
    ...
    "notAssigned": "您目前尚未被分配到任何工作區。\n請聯絡您的管理員以申請工作區的存取權限。",
    "goToWorkspace": "前往 \"{{workspace}}\""
  },
  "telegram": {
    "title": "Telegram 機器人",
    "description": "將您的 AnythingLLM 實例連接到 Telegram，以便您可以在任何裝置上與您的工作空間進行對話。",
    "setup": {
      "step1": {
        "title": "第一步：建立您的 Telegram 機器人",
        "description": "在 Telegram 中開啟 @BotFather，將 \"<code>/newbot\" 訊息發送至 <code>@BotFather</code>，按照指示操作，並複製 API 令牌。",
        "open-botfather": "開啟 BotFather",
        "instruction-1": "1. 點擊連結或掃描 QR 碼",
        "instruction-2": "2. 將 <code>/newbot</code> 傳送至 <code>@BotFather</code>",
        "instruction-3": "3. 為您的機器人選擇一個名稱和使用者名稱。",
        "instruction-4": "4. 複製您收到的 API 令牌"
      },
      "step2": {
        "title": "步驟 2：連接您的機器人",
        "description": "請將您從 @BotFather 處獲得的 API 令牌複製並貼上，然後選擇一個預設的工作空間，讓您的機器人與其對話。",
        "bot-token": "機器人代幣",
        "connecting": "正在連接...",
        "connect-bot": "連線機器人"
      },
      "security": {
        ...
        "obscure-username": "使用一個不顯眼的機器人帳號名稱，以降低被發現的機會。"
      },
    ...
    "users": {
      "pending-description": "等待驗證的使用者。請將這裡顯示的配對碼與他們在 Telegram 聊天中顯示的配對碼對齊。",
      "unknown": "未知的"
    }
  }
}
