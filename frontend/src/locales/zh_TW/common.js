...
const TRANSLATIONS = {
  "onboarding": {
    ...
    "userSetup": {
      ...
      "teamHint": "預設情況下，您將是唯一的管理員。完成入職後，您可以創建和邀請其他人成為使用者或管理員。不要遺失您的密碼，因為只有管理員可以重置密碼。"
    },
    "data": {
      "title": "資料處理與隱私",
  ...
  "general": {
    ...
    "delete": {
      ...
      "confirm-end": "工作區。這將會移除向量資料庫中的所有向量嵌入。\n\n原始檔案將保持不變。此動作無法復原。"
    }
  },
  "chat": {
    "llm": {
      "title": "工作區 LLM 提供者",
      "description": "此工作區將使用的特定 LLM 提供者與模型。預設情況下，它會使用系統 LLM 提供者和設定。",
      "search": "搜尋所有 LLM 提供者"
    },
    "model": {
      "title": "工作區對話模型",
    ...
    "prompt": {
      "title": "提示詞",
      "description": "將在此工作區中使用的提示詞。定義 AI 產生回應的上下文和指示。您應該提供精心設計的提示詞，以便 AI 可以產生相關且準確的回應。",
      "history": {
        "title": "系統提示歷史記錄",
        "clearAll": "清除全部",
  ...
  "vector-workspace": {
    ...
    "reset": {
      "reset": "重設向量資料庫",
      "resetting": "清除向量中...",
      "confirm": "您即將重設此工作區的向量資料庫。這將會移除目前所有已嵌入的向量。\n\n原始檔案將保持不變。此動作無法復原。",
      "error": "無法重設工作區向量資料庫！",
      "success": "工作區向量資料庫已重設！"
    }
  },
  "agent": {
    "performance-warning": "不直接支援工具呼叫的 LLM 的效能，高度取決於模型的功能和精確度。某些功能可能受限或無法使用。",
    "provider": {
    ...
    "skill": {
      "rag": {
        "title": "RAG 與長期記憶體",
        "description": "允許智慧代理人利用您的本機文件來回答查詢，或要求智慧代理人「記住」內容片段，以利長期記憶體擷取。"
      },
      "view": {
        "title": "檢視與摘要文件",
      ...
      "web": {
        "title": "即時網路搜尋與瀏覽",
        "description": "透過連接到網路搜尋引擎 (SERP) 提供商，讓您的代理能夠在網上搜尋，以回答您的問題。"
      },
      "sql": {
        "title": "SQL 連接器",
        "description": "讓您的代理能夠利用 SQL 查詢來回答您的問題，只需連接到不同的 SQL 數據庫提供者即可。"
      },
      "default_skill": "預設情況下，此功能已啟用，但您可以選擇關閉，以防止其被應用程式使用。"
    }
  },
  "recorded": {
    "title": "工作區對話紀錄",
  ...
  "llm": {
    "title": "LLM 偏好設定",
    "description": "這些是您偏好的 LLM 對話與嵌入提供者的憑證和設定。確保這些金鑰是最新且正確的，否則 AnythingLLM 將無法正常運作。",
    "provider": "LLM 提供者",
    "providers": {
      "azure_openai": {
  ...
  "transcription": {
    ...
    "provider": "語音轉錄提供者",
    "warn-start": "在記憶體或處理器資源有限的電腦上使用本機 Whisper 模型，處理媒體檔案時可能會造成 AnythingLLM 停頓。",
  ...
  "embedding": {
    ...
    "desc-end": "嵌入是將文字轉換成向量的過程。這些憑證是用於將您的檔案和提示詞轉換成 AnythingLLM 可以處理的格式。",
    "provider": {
      "title": "向量嵌入提供者"
    }
  },
  ...
  "vector": {
    "title": "向量資料庫",
    "description": "這些是您的 AnythingLLM 系統運作方式的憑證和設定。確保這些金鑰是最新且正確的，這點非常重要。",
    "provider": {
      "title": "向量資料庫提供者",
  ...
  "embeddable": {
    "title": "可嵌入對話小工具",
    "description": "可嵌入對話小工具是與單一工作區連結的公開對話介面。這讓您可以建置工作區，然後發布到全世界。",
    "create": "建立嵌入",
    "table": {
      "workspace": "工作區",
      "chats": "已傳送對話",
  ...
  "connectors": {
    ...
    "github": {
      ...
      "token_information": "若未填寫 <b>GitHub 存取權杖</b>，此資料連接器僅能收集倉庫的 <b>頂層</b> 檔案，因 GitHub 的公共 API 速率限制。",
    ...
    "gitlab": {
      ...
      "branch_loading": "-- 載入可用分支 --",
      "branch_explained": "您希望收集檔案的分支。",
      "token_information": "若未填寫 <b>GitLab 存取權杖</b>，此資料連接器僅能收集倉庫的 <b>頂層</b> 檔案，因 GitLab 的公共 API 速率限制。",
    ...
    "confluence": {
      ...
      "bypass_ssl_explained": "啟用此選項，以繞過自簽憑證的 SSL 憑證驗證，適用於您自行託管的 Confluence 實例。"
    },
    "manage": {
      "documents": "文件",
    ...
    "upload": {
      ...
      "fetching": "正在擷取...",
      "fetch-website": "擷取網站",
      "privacy-notice": "這些檔案將上傳到此 AnythingLLM 實例中的文件處理器。這些檔案不會發送或共享給第三方。"
    },
    "pinning": {
      "what_pinning": "什麼是文件固定？",
      "pin_explained_block1": "當您在 AnythingLLM 中<b>固定</b>一個文件時，我們會將該文件的所有內容注入到您的提示窗口中，讓您的 LLM 完全理解。",
      "pin_explained_block2": "這對於<b>大範圍模型</b>或對知識庫至關重要的小型文件效果最佳。",
      "pin_explained_block3": "如果您沒有從 AnythingLLM 預設獲得理想的答案，那麼固定是一個輕鬆獲得更高質量答案的方法。",
      "accept": "好的，明白了"
    },
    "watching": {
      "what_watching": "觀看文件有何作用？",
      "watch_explained_block1": "當您在 AnythingLLM 中<b>觀看</b>一個文件時，我們會<i>自動</i>定期同步該文件的內容，並在每個管理該文件的工作區中自動更新內容。",
  ...
  "chat_window": {
    ...
    "agent_skills_disabled_in_session": "在執行代理時，無法修改技能。請先使用 /exit 命令結束本次執行。",
    "start_agent_session": "開始代理會談",
    "use_agent_session_to_use_tools": "您可以使用聊天中的工具，只需在您的指令開頭加上'@agent'，即可開始與代理的對話。"
  },
  "profile_settings": {
    "edit_account": "編輯帳戶",
  ...
  "customization": {
    ...
    "items": {
      ...
      "render-html": {
        ...
        "description": "將 HTML 格式的回應嵌入到助理的回應中。\n這可以顯著提高回應品質，但也可能帶來潛在的安全風險。"
      }
    }
  },
  "main-page": {
  ...
  "security": {
    ...
    "multiuser": {
      ...
      "enable": {
        "is-enable": "多使用者模式已啟用",
        "enable": "啟用多使用者模式",
        "description": "預設情況下，您將是唯一的管理員。身為管理員，您需要為所有新使用者或管理員建立帳號。請勿遺失您的密碼，因為只有管理員使用者可以重設密碼。",
        "username": "管理員帳號使用者名稱",
        "password": "管理員帳號密碼"
      }
    },
    "password": {
      "title": "密碼保護",
      "description": "使用密碼保護您的 AnythingLLM 系統。如果您忘記這個密碼，將沒有復原方法，因此請務必儲存此密碼。",
      "password-label": "實例密碼"
    }
  },
  "home": {
