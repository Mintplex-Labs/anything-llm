# How to use Dockerized Obaradai LLM

Use the Dockerized version of ObaradaiLLM for a much faster and complete startup of ObaradaiLLM.

### Minimum Requirements

> [!TIP]
> Running ObaradaiLLM on AWS/GCP/Azure?
...
\*ObaradaiLLM by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

\*ObaradaiLLM by default embeds text on instance privately [Learn More](../server/storage/models/README.md)

## Recommend way to run dockerized ObaradaiLLM!
...
> the docker container using ObaradaiLLM as `localhost:xxxx` will not resolve for the host system.
>
...
```shell
docker pull wmwijaya/obaradai-llm
```

<table>
<tr>
<th colspan="2">Mount the storage locally and run ObaradaiLLM in Docker</th>
</tr>
<tr>
<td>
  Linux/MacOs
...
Go to `http://localhost:3001` and you are now using ObaradaiLLM! All your data and progress will persist between
...
The integrations below are templates or tooling built by the community to make running the docker experience of ObaradaiLLM easier.

### Use the Midori AI Subsystem to Manage ObaradaiLLM

Follow the setup found on [Midori AI Subsystem Site](https://io.midori-ai.xyz/subsystem/manager/) for your host OS
After setting that up install the ObaradaiLLM docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to http://127.0.0.1:11434 you should put `http://host.docker.internal:11434` into the connection URL in ObaradaiLLM.
