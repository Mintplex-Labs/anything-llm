...
if (!envFileExists) {
  console.log(
    chalk.redBright("[ABORT]"),
    "You do not have an .env file in your ./docker/ folder. You need to create it first."
  );
  console.log(
    "You can start by running",
    chalk.cyan("cp -n ./docker/.env.example ./docker/.env")
  );
  exit(1);
}
...
const updatedTemplateString = templateString.replace(
  REPLACEMENT_KEY,
  formattedSettings
);
...
console.log(
  ...
  "Deploy AnythingLLM on GCP Deployment Manager using your template document."
);
console.log(
  chalk.greenBright("File Created:"),
  "gcp_deploy_anything_llm_with_env.yaml in the output directory."
);
console.log(
  chalk.blueBright("[INFO]"),
  "Refer to the GCP Deployment Manager documentation for how to use this file."
);
